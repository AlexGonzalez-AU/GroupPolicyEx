function Patch-GpoGptTmplFile_RestrictedGroups {
    [CmdletBinding()]
    ...
    begin {
        ...
        # Get the sids for 'Domain Member Admins' group
        [array]$SidStrings = @()
        $SidStrings += (Get-ADGroup -Identity "Domain Member Admins").Sid.Value
    ...
    process {
        ...
        if (Test-Path $infPath) {
            ...
            for ($i = 0; $i -lt $inf.length; $i++) {
                ...
                if ($section) {
                    if ($line.StartsWith('*S-1-5-32-544__Members')) {
